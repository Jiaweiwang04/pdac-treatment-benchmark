...
当前仅推进Track A，Track B冻结，不因其临床指标缺失设置新的纳入门槛。本轮依据用户确认，将515名主集合患者分为435名开发患者和80名Core测试患者，再从开发集中抽取25名Pilot患者。主标签继续沿用[四类标签定义](treatment_label_definition_v2.0.md)，知识截止日固定为2026-09-12；划分执行日期为2026-09-13，不改变知识截止日。
...
运行 `python -B code/scripts/run_v2_0.py split` 重建本阶段；运行 `python -B code/scripts/run_v2_0.py all` 重建六阶段并核对测试、来源及文档。配置见[划分配置](../../../code/config/v2.0/patient_split.json)，名单锁定见[锁定文件](../../../data/processed/v2.0/06_patient_split/patient_split_lock_v2.0.json)。已有锁定与输入、规则或名单不一致时程序报错，不覆盖重抽；需另行讨论有版本记录的修订。锁定是流程与校验控制，不是操作系统访问权限或已完成盲法审核的证明。
...
Track A主实验评价四类标签、证据支持、候选识别及无法判定的处理。ECOG、剂量、胆红素和毒性等Track B完整安全评价不在本轮主实验范围。本次是同来源队列内、按患者分组的常规测试设计，不等于证据时间外、机构外或独立来源外测试。
