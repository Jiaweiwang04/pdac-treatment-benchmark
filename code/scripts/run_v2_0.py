...
from pdac_benchmark.v2_0.track_a import run as run_candidates


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("action", choices=["all", "audit", "timeline", "candidates", "check"], nargs="?", default="all")
    ...
    if args.action in {"all", "candidates"}:
        result = run_candidates(BASE / "code/config/v2.0/track_a.json")
